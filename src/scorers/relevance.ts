...
const RELEVANCE_PROMPT = `You are an expert at evaluating response relevance.
...
const ANSWER_RELEVANCE_PROMPT = `You are an expert at evaluating Q&A quality.
 ...
 * ```
 */
export function Relevance(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'relevance', RELEVANCE_PROMPT);
}
 ...
 * @example
 * ```typescript
 * const answerRelevance = AnswerRelevance({
 *   client: { apiKey: config.apiKey, baseUrl: config.baseUrl },
 * });
 ...
 * ```
 */
export function AnswerRelevance(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'answer_relevance', ANSWER_RELEVANCE_PROMPT);
}
