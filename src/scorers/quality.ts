...
const COHERENCE_PROMPT = `You are an expert at evaluating text coherence and clarity.
...
const FLUENCY_PROMPT = `You are an expert linguist evaluating text fluency.
...
const COMPLETENESS_PROMPT = `You are an expert at evaluating response completeness.
 ...
 * });
 * const result = await coherence({
 *   output: 'The sky is blue because of Rayleigh scattering...',
 *   expected: 'Why is the sky blue?',
 * });
 * ```
 */
export function Coherence(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'coherence', COHERENCE_PROMPT);
}
 ...
 *   client: { apiKey: config.apiKey, baseUrl: config.baseUrl },
 * });
 * const result = await fluency({ output: 'This is a well-written sentence.' });
 * ```
 */
export function Fluency(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'fluency', FLUENCY_PROMPT);
}
 ...
 * ```
 */
export function Completeness(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'completeness', COMPLETENESS_PROMPT);
}
