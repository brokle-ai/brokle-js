...
const FACTUALITY_PROMPT = `You are an expert fact-checker. Evaluate whether the following output is factually correct.
...
const HALLUCINATION_PROMPT = `You are an expert at detecting hallucinations in AI-generated text.
 ...
 * ```
 */
export function Factuality(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'factuality', FACTUALITY_PROMPT);
}
 ...
 *   client: { apiKey: config.apiKey, baseUrl: config.baseUrl },
 * });
 * const result = await hallucination({
 *   output: 'According to the document, sales increased 50%.',
 *   expected: 'The document states sales increased 25% year over year.',
 * });
 * ```
 */
export function Hallucination(options: BaseEvaluatorOptions): Scorer {
  return createEvaluatorFromPrompt(options, 'hallucination', HALLUCINATION_PROMPT);
}
